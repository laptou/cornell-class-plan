...
interface Course {
    code: [string, number] | [string, number, string, number];
    name: string;
    credits: number;
    type: CourseType[];
}
...
type CourseType =
    'core' | 'distribution' | 'major' | 'minor' | 'elective' |
    'stem' | 'liberal' | 'fws' | 'language' | 'pe' | 'other';
...
function make(type: string, classes?: string[], ...content: (Node | string)[]) {
  const el = document.createElement(type);
  if (classes) el.classList.add(...classes);
  if (content) content.forEach(v => el.append(v));
  return el;
}

const { body } = document;
for (const year of plan) {
  body.append(make('section', ['year'],
    make('h2', [], year.name),
    make('div', ['semesters'], ...year.semesters.map(semester =>
      make('section', ['semester'],
        make('h3', [], semester.name),
        make('ul', ['courses'], ...(semester.courses.length > 0 ? [
          ...semester.courses.map(course =>
            make('li', ['course', ...course.type],
              make('span', ['code'], course.code.length === 4 ?
                `${course.code[0]} ${course.code[1]}/${course.code[2]} ${course.code[3]}` :
                `${course.code[0]} ${course.code[1]}`),
              make('span', ['name'], course.name),
              make('span', ['credits'], course.credits.toString(10))
            )),
